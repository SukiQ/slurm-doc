...
| AccountingStorageEnforce      | 控制对作业提交施加的关联级别                                 |
...
| JobAcctGatherType             | JobAcctGather 插件根据配置的插件收集任务级别的内存、CPU、IO、互联、能量和 GPU 使用信息。此参数将控制某些指标的收集方式 --  jobacct_gather/cgroup (推荐)：通过读取任务的 cgroup 目录接口收集 CPU 和内存统计信息 -- jobacct_gather/linux：通过读取 procfs 收集 CPU 和内存统计信息 -- jobacct_gather/none：默认值。不收集任何记账数据 |
| JobAcctGatherFrequency        | 作业会计和剖析采样间隔。支持的格式为：`JobAcctGatherFrequency=<datatype>=<interval>` 默认任务采样间隔为 30 秒。其他间隔的默认值为 0 |
...
| PriorityType                  | 指定用于建立作业调度优先级的插件 -- priority/basic：作业按先到先服务（FIFO）方式进行评估 -- priority/multifactor 作业根据多种因素分配优先级，包括大小、年龄、公平共享等。在非 FIFO 调度时，作业的优先级按以下顺序排列：1. 可以被抢占的作业 2. 具有高级预留的作业 3. 分区 PriorityTier 4. 作业优先级 5. 作业提交时间 6. 作业ID |
...
| SelectType                    | 资源选择算法类型。变更时将丢失所有作业信息 -- select/cons_tres：个别分配可消耗资源 -- select/linear：按顺序分配整个节点，适用于异构集群 |
| SelectTypeParameters          | 取决于 SelectType 的参数选项 -- CR_CPU：CPUs 是可消耗资源，配置每个节点的 CPU 数量，影响作业分配的局部性 -- CR_CPU_Memory：CPUs 和内存是可消耗资源，建议设置 DefMemPerCPU 以优化作业分配 -- CR_Core：Cores 是可消耗资源，多个作业不能在同一核心上分配线程 -- CR_Core_Memory：Cores 和内存是可消耗资源，建议设置 DefMemPerCPU -- CR_ONE_TASK_PER_CORE：默认每个核心分配一个任务，不能与 CR_CPU* 选项一起使用 -- CR_CORE_DEFAULT_DIST_BLOCK：默认按块分配核心，最小化资源使用。可通过参数覆盖 -- CR_LLN：将资源分配给最空闲的节点，适合串行作业，但可能导致资源碎片化 -- CR_Pack_Nodes：紧密打包任务，而不是均匀分配到多个节点 -- LL_SHARED_GRES：为共享 GRES 资源分配时，优先选择负载最轻的设备 -- CR_Socket：Sockets：是可消耗资源，多个作业不能在同一插槽上分配 -- CR_Socket_Memory：内存和插槽是可消耗资源，建议设置 DefMemPerCPU -- CR_Memory：内存是可消耗资源，建议设置 DefMemPerCPU -- MULTIPLE_SHARING_GRES_PJ：允许每个节点多个共享 GRES，满足共享要求 -- ENFORCE_BINDING_GRES：默认启用 --gres-flags=enforce-binding，支持覆盖 -- ONE_TASK_PER_SHARING_GRES：默认启用 --gres-flags=one-task-per-sharing，支持覆盖 |
